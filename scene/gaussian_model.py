...
class GaussianModel(nn.Module):
    ...
    def __init__(self, sh_degree : int):
        ...
        self.voxel_index = torch.empty(0)   # ! added，对应vdbfusion中每个voxel的index
    ...
    def create_from_pcd2_tensor(self, points, colors, rots_, scales_, z_vals_, trackable_idxs, voxel_idx=None):
        ...
        if voxel_idx is not None:
            self.voxel_index = torch.tensor(voxel_idx, device="cuda", dtype=torch.int32)
    ...
    def add_from_pcd2_tensor(self, points, colors, rots_, scales_, z_vals_, trackable_idxs, voxel_idx=None):
        ...
        if voxel_idx is not None:
            self.voxel_index = torch.concat([self.voxel_index, torch.tensor(voxel_idx, device="cuda", dtype=torch.int32)], dim=0)
    ...
    def prune_points(self, mask):
        ...
        # 如果self.voxel_index不为空，则删除对应的voxel_index
        if self.voxel_index.size(0) != 0:
            self.voxel_index = self.voxel_index[valid_points_mask]
    ...
    def save_ply(self, path):
        ...
        # voxel_index = self.voxel_index.cpu().numpy()
        ...
        print("!!!!!!!! Successfully saved ply file !!!!!!!!")
        
        
    def save_npz(self, path, mapping_cams):
        '''
        ...
        print("!!!!!!!! Successfully saved npz file !!!!!!!!")
