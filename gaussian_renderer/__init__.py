...
def render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None):
    ...
    """
    # Create zero tensor for screen-space means, enabling gradient computation
    ...
    # Compute tangent of field of view for rasterization
    ...
    # Covariance: use precomputed if available, otherwise compute from scaling/rotation
    scales = None
    rotations = None
    cov3D_precomp = None
    ...
    # Color: use override, precompute from SHs in Python, or let rasterizer handle SH->RGB
    shs = None
    colors_precomp = None
    ...
    # Only visible Gaussians (radii > 0) are considered for updates
    return {
...
def render_2(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None, training_stage=0):
    """
    Render the scene with optional lower resolution for training stages.
    """
    screenspace_points = torch.zeros_like(pc.get_xyz, dtype=pc.get_xyz.dtype, requires_grad=True, device="cuda")
    try:
        screenspace_points.retain_grad()
    ...
    # Adjust resolution for training stage
    if training_stage == 0:
...
def render_3(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None, training_stage=0):
    """
    Render the scene for batched camera parameters (e.g., multi-view training).
    """
    screenspace_points = torch.zeros_like(pc.get_xyz, dtype=pc.get_xyz.dtype, requires_grad=True, device="cuda")
    try:
        screenspace_points.retain_grad()
